...
public interface IGDriveTrashOperations
{
    ...
    /// <summary>
    /// Retrieves all items currently located in the Google Drive trash, across all pages.
    /// </summary>
    ...
    /// <exception cref="Google.GoogleApiException">Thrown when the Google Drive API returns an error.</exception>
    /// <exception cref="OperationCanceledException">Thrown when the operation is cancelled via the cancellation token.</exception>
    Task<IReadOnlyList<GoogleFile>> ListAsync(int pageSize = 50, CancellationToken cancellationToken = default);
}
